...
This kind of representation makes it easier to see the expression’s structure than when we were writing it all on one line.

It is possible to make this even more concise by not writing the ones at all. This works because the ones always appear in predictable places in these representations. For example, consider this more complex representation, of the number <Number num="1246208"/>:
...
Here’s how it looks without the ones:

<div class="rpns text-rpns" id="no-one">
<Times>
  <Prime>
    <One/>
    <Times>
...
If you add ones above/below/between all the horizontal lines (except in the places where there is a multiplication <span style="display: inline-block; vertical-align: middle; width: 0.24em; height: 1.5em; position: relative; bottom: 0.05em; margin: 0 0.2em; background-color: var(--brown)"/> bar), that will put them all in the right places. This is true for any number. So we can leave out the ones, which allows us to write this more compactly:
...
Just for aesthetic purposes, I’ll replace the shortest horizontal lines with dots:
...
If you wanted to compact this even more, you could remove the vertical multiplication bars as they aren’t actually necessary to interpret the number — but I’ll keep them, since they make things clearer.
...
{/* a much larger number, like <span id="monster-base10"><Number num="808017424794512875886459904961710757005754368000000000"/></span> */}
