...
(In <Group>A ◊ B</Group>, A is the *index* and B is the *exponent*.)
...
The system can be made more concise by removing the ones and just writing the bars. This does not lead to any ambiguity: if a horizontal bar has nothing in the space above it, then the index is assumed to be 1; if it has nothing in the space below it, the exponent is assumed to be 1. So that above example becomes:
...
- **1:** a blank space
- **A ◊ B:** a shape enclosing A (the index) and touching B (the exponent)
- **A × B:** A and B next to each other, not touching

Using this system, a number can be represented as a composition of shapes.

For some examples of how these rules work, let’s use circles labeled A and B as stand-ins for compositions that represent numbers:

<Img src='rpns/shapes/A*B.svg' uses='f' id="test" alt="" scale={3}/>

That represents <Group>A × B</Group>, since they are not touching. There is no requirement to arrange them in any particular way — the following also represents <Group>A × B</Group> (or equivalently <Group>B × A</Group>):

<Img src='rpns/shapes/A*B alternate.svg' uses='f' id="test" alt="" scale={3}/>

Here’s one way to write <Group>A ◊ B</Group> (the A<span class="ord">th</span> prime to the power of B), by drawing a shape that encloses A and touches B:
...
Since 1 is represented by a blank space or the lack of a shape, <Group>A ◊ 1</Group> is written with a shape enclosing A and touching no thing:

<Img src='rpns/shapes/A◊1.svg' uses='f' id="test" alt="" scale={3}/>

And <Group>1 ◊ A</Group> is written with a shape containing nothing, and touching A:

<Img src='rpns/shapes/1◊A.svg' uses='f' id="test" alt="" scale={3}/>

Now let’s see some actual numbers, without placeholder variables. The simplest is 2: <Group>1 ◊ 1</Group> is written with a shape enclosing nothing and touching nothing:
...
There, the outer circle is not touching anything, so the exponent is taken to be 1. To write <Group>2 ◊ 2</Group> (= 9), make that outer circle touch a shape (any shape) representing 2:
...
- and touching the exponent 2 (the semicircle),
<Glue/>
...
- and touching the exponent 3 (the circle around the square),
<Glue/>
which represents <Group>1 ◊ 3 = 8.</Group>

To avoid this kind of ambiguity, I added a rule specifying that *when two shapes touch each other, the larger one contains the index, and the smaller one is the exponent.* So here, the first interpretation (9) is correct. If you wanted to write 8, you would make the semicircle larger:

<Img src='rpns/shapes/8.svg' uses='f' id="test" alt="" scale={3}/>

And if more than two shapes touch each other, then the largest one is taken as the shape containing the index, and all the other shapes compose the exponent. For an example of this, I’ll start by writing 9 again (this time written with only rectangles, for variety):

<Img src='rpns/shapes/9 rect.svg' uses='f' id="test" alt="" scale={3}/>

To write <Group>1 ◊ 9</Group> (the 1<span class="ord">st</span> prime to the power of 9, 2⁹, 512), we can draw a bigger rectangle touching this:

<Img src='rpns/shapes/512 rect.svg' uses='f' id="test" alt="" scale={3}/>

Since that new rectangle is the largest shape, its content (nothing, meaning 1) is taken to be the index, and everything it’s touching (9) is taken to be the exponent.

Here, all



 larger rectangle (enclosing the index 1) touches both of the larger rectangles that make up 9. But it doesn’t have to — 512 could also be written like this, with that largest rectangle on the left:

<Img src='rpns/shapes/512 linear.svg' uses='f' id="test" alt="" scale={3}/>

This composition breaks the rule that the enclosing shape must be l

Note here that the left rectangle (enclosing 1, touching 9) is smaller than the composition representing 9: this breaks the rule that the enclosing shape must
