...
To convert an encoding to an image, let’s say that 0 is white and 1 is black. (If you’re a designer I know this might be the opposite of what you’re used to, but remember that “black” is really just shorthand for “a point inside the shape” while “white” means “outside the shape” — 1 is inside, 0 is outside.) 
...
<Img class="shape-building" src="resolution-code/diagrams/sample shape.svg" alt=""/>
...
<Img class="shape-building" src="resolution-code/diagrams/iteration 1 0.svg" alt=""/>
...
<Img class="shape-building" src="resolution-code/diagrams/iteration 1 1.svg" alt=""/>

The top right pixel covers a region that’s partly black and partly white. We need more detail to represent what’s going on here, so we *divide it into four smaller pixels* that we’ll return to later.

<Img class="shape-building" src="resolution-code/diagrams/iteration 1 2.svg" alt=""/>

We proceed to the next row: the bottom right pixel is fully white, while the bottom right has both black and white, so we divide it as well:

<Img class="shape-building" src="resolution-code/diagrams/iteration 1 3.svg" alt=""/>




Now we’ve reached the bottom of the image. At this point we’ve already defined half of the image — the half that’s fully outside the shape.
But there is the other half that needs more detail. So we go back to the start and start a second iteration, doing the same thing at a smaller scale.
Reading from left to right, then top to bottom
Split, In, Split, In, Split, Split, Out, Out
...
S, I, S, I, S, I, S, S, O, S, S, I, O, O, S, S.
And so on. We could do this indefinitely. It produces an ever-lengthening sequence:
...
If you got this sequence, you could use it to reconstruct the shape in as much detail as you’d like.
